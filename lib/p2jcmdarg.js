...
class CLIArgParser {
    ...
    showHelp() {
        ...
        for (const [key, value] of Object.entries(this.#aliases)) {
            if (key === value.name) {
                helpMsg += `--${value.name}\t ${value.description}\n`;
            } else {
...
export const yargs = new CLIArgParser(process.argv.slice(2))
	...
	.alias('q', 'quiet', '(optional) when specified, will suppress all non-error output.')
	.alias('json', 'json', '(optional) when specified, will output parsed JSON to stdout instead of a file.');
